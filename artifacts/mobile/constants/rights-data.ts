...
// ── Static English arrays (kept for backward-compat; use factory functions in UI) ──
...
export const QUIZ_QUESTIONS: QuizQuestion[] = [
  {
    ...
    options: ['License only', 'License, registration, and proof of insurance', 'Nothing — you can remain silent', 'Only your name'],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: ['Just stay quiet without saying anything', 'Say "I plead the fifth" repeatedly', 'Clearly say "I am invoking my right to remain silent"', 'Walk away from the officer'],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: ['Never — it is always illegal', 'Only with written permission', 'Yes, it is a protected First Amendment right', 'Only in California'],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: ['Yes, always', 'Only if they show any badge', 'No — only if they have a signed judicial warrant', 'Yes, if they identify themselves as officers'],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: ['Never — you must wait to be released', 'Only after 15 minutes', 'At any point during the encounter', 'Only if you are not under arrest'],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: ['Physically block them from entering', 'Comply and challenge the entry in court afterward', 'Call 911 immediately', 'Lock all the doors'],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: ['Agricultural regulations on police property', 'Evidence gathered from an illegal search can be excluded in court', 'A rule about planting evidence', 'Police must disclose all evidence before trial'],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: ['24 hours', '48 hours', '72 hours', 'Until trial'],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: ['Yes, it looks suspicious', 'Only at night', 'No — your refusal cannot be used as probable cause', 'Yes, in high-crime areas'],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: ['Immediately upon arrest', 'Before any police questioning ever', 'Before a custodial interrogation', 'Only at the police station'],
    correctIndex: 2,
...
// ── Factory functions — call these inside React components to get translated data ──

/** Returns rights categories with titles and rights strings in the active language. */
export function getRightsCategories(t: (key: I18nKey) => string): RightsCategory[] {
  return RIGHTS_CATEGORIES.map((cat) => ({
    ...cat,
    title:  t(`rights.cat_${cat.id}` as I18nKey),
    rights: cat.rights.map((_, i) => t(`rights.${cat.id}_r${i + 1}` as I18nKey)),
  }));
}

/** Returns quiz questions with translated question text, options, and explanations. */
export function getQuizQuestions(t: (key: I18nKey) => string): QuizQuestion[] {
  return QUIZ_QUESTIONS.map((q, qi) => {
    const n = qi + 1;
    return {
      ...q,
      question:    t(`rights.q${n}_q`   as I18nKey),
      options:     q.options.map((_, oi) => t(`rights.q${n}_a${oi}` as I18nKey)),
      explanation: t(`rights.q${n}_exp` as I18nKey),
    };
  });
}
