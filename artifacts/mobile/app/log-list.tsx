...
<div class="card" id="lock-card">
  ...
  <div class="note">Encrypted by CivicShield Pro · AES-256-CBC / PBKDF2-SHA256 (100 000 iterations)<br/>Works offline – decryption library is bundled inside this file.</div>
</div>
<div id="doc"></div>
<script>
...
function timingSafeEqual(a,b){if(a.length!==b.length)return false;var d=0;for(var i=0;i<a.length;i++)d|=a.charCodeAt(i)^b.charCodeAt(i);return d===0;}
function unlock(){
  ...
  setTimeout(function(){
    try{
      ...
      if(p.v!==2)throw new Error('unsupported version');
      ...
      var km=CryptoJS.PBKDF2(pwd,salt,{keySize:16,iterations:100000,hasher:CryptoJS.algo.SHA256});
      var kmHex=km.toString(CryptoJS.enc.Hex);
      var kenc=CryptoJS.enc.Hex.parse(kmHex.slice(0,64));
      var kmac=CryptoJS.enc.Hex.parse(kmHex.slice(64,128));
      // Verify Encrypt-then-MAC before decrypting
      var expected=CryptoJS.HmacSHA256(p.s+p.i+p.c,kmac).toString(CryptoJS.enc.Hex);
      if(!timingSafeEqual(expected,p.h))throw new Error('bad mac');
      var decrypted=CryptoJS.AES.decrypt(p.c,kenc,{iv:iv,mode:CryptoJS.mode.CBC,padding:CryptoJS.pad.Pkcs7});
